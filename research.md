...
```
web-crawler-books/
├── scraper.py       # 카테고리 기반 크롤링 + CSV/Excel 저장 로직, CLI 진입점
...
## AI 기반 추출 기능 (ai_extractor.py) 추가 내역
...
키, aistudio.google.com에서 구글 계정으로 발급).

동작 방식: 페이지의 스크립트/스타일/nav/footer를 제거한 뒤 순수 텍스트만
추출(최대 12,000자) → JSON 강제 응답(`response_mime_type=application/json`)으로
Gemini에 요청 → 결과를 기존 저장 함수와 동일한 스키마로 정규화.

실제 테스트 결과(태블릿 카테고리, 21건): 상품명·가격·리뷰 수는 정확히
추출됐으나, **평점은 전부 0으로 나옴** — 이 사이트의 별점이 텍스트가
아니라 아이콘(`ws-icon-star`)으로만 표시되어 있어, 텍스트 추출 단계에서
정보 자체가 사라지기 때문. `data-rating` 속성을 직접 읽는 기존
셀렉터 방식과 달리, AI 방식은 "눈에 보이는 텍스트"에만 의존하는 근본적
한계가 있음 — 아이콘/이미지로만 표현된 정보는 프롬프트를 아무리
다듬어도 원천적으로 복구 불가능(텍스트 추출 단계에서 이미 유실됨).

## 위험요소 (AI 기능 추가분)
| 구분 | 내용 |
|---|---|
| 비용/속도 | API 호출 1회당 비용·지연이 발생하고 무료 등급은 분당 요청 수 제한이 있어, 대량 상품에는 부적합 (소량/1회성 낯선 사이트 탐색용으로 한정) |
| 데이터 정확도 | 아이콘/이미지 기반 정보(별점 등)를 못 읽음. 또한 LLM 특성상 동일 입력에도 결과가 완전히 결정적이지 않을 수 있음 |
| 비밀키 관리 | `.env` 파일이 실수로 커밋되면 API 키가 유출됨 — `.gitignore`에 `.env` 추가로 1차 방어했으나, 협업 시에도 주의 필요 |
| 모델명 변경 | 실제 테스트 중 `gemini-2.5-flash`가 신규 사용자에게 더 이상 제공되지 않아 `gemini-3.6-flash`로 교체한 이력 있음 — Google이 모델을 자주 세대교체하므로 향후에도 모델명이 만료될 수 있음(에러 메시지에 대체 모델명이 안내되니 그때그때 갱신 필요) |
...
2. **에러 처리 수준** — 포트폴리오 데모 수준(현재 상태 유지)으로 둘지,
   ...
   보강할지 아직 미정
3. **대량 크롤링 대응** — ~~Scrapy 전환~~ 대신 **AI 기반 추출(`--ai-url`)을
   먼저 추가함** (셀렉터 없는 사이트 대응이라는 다른 각도의 확장). 대량
   처리 성능 자체에 대한 Scrapy 전환 여부는 여전히 미정
